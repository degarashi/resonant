...
namespace rs {
	...
	void ABuffer_depAL::writeBuffer(const AFormatF& af, const void* src, size_t len) {
		ALEC_D(Trap, alBufferData, _id, AsALFormat(af), src, len, af.freq);
	}
	...
	ASource_depAL::~ASource_depAL() {
		if(_id != 0) {
			ALEC_D(Trap, alSourceStop, _id);
			ALEC_D(Trap, alSourcei, _id, AL_BUFFER, 0);
			ALEC_D(Trap, alDeleteSources, 1, &_id);
		}
	}
	void ASource_depAL::play() { ALEC_D(Trap, alSourcePlay, _id); }
	void ASource_depAL::reset() {
		ALEC_D(Trap, alSourceStop, _id);
		ALEC_D(Trap, alSourceRewind, _id);
	}
	void ASource_depAL::pause() { ALEC_D(Trap, alSourcePause, _id); }
	void ASource_depAL::update(bool bPlaying) {
		// もし再生中なのにバッファの途切れなどでSTOPPEDになっていたらPLAYINGにセット
		if(bPlaying) {
			ALint state;
			ALEC_D(Trap, alGetSourcei, _id, AL_SOURCE_STATE, &state);
			if(state == AL_STOPPED)
				ALEC_D(Trap, alSourcePlay, _id);
		}
	}
	void ASource_depAL::setGain(float vol) {
		ALEC_D(Trap, alSourcef, _id, AL_GAIN, vol);
	}
	...
	void ASource_depAL::timeSeek(float t) {
		ALEC_D(Trap, alSourcef, _id, AL_SEC_OFFSET, t);
	}
	void ASource_depAL::pcmSeek(int64_t t) {
		// floatでは精度が不足するかもしれないが、int32_tだと桁が足りないかもしれない
		ALEC_D(Trap, alSourcef, _id, AL_SAMPLE_OFFSET, static_cast<ALfloat>(t));
	}
	void ASource_depAL::enqueue(ABuffer_depAL& buff) {
		ALuint buffID = buff.getID();
		ALEC_D(Trap, alSourceQueueBuffers, _id, 1, &buffID);
	}
	int ASource_depAL::getUsedBlock() {
		int nproc;
		ALEC_D(Trap, alGetSourcei, _id, AL_BUFFERS_PROCESSED, &nproc);
		if(nproc > 0) {
			ALuint ids[MAX_AUDIO_BLOCKNUM];
			ALEC_D(Trap, alSourceUnqueueBuffers, _id, nproc, ids);
		}
		return nproc;
	}
	...
	void ASource_depAL::setRelativeMode(bool bRel) {
		ALEC_D(Trap, alSourcei, _id, AL_SOURCE_RELATIVE, bRel ? AL_TRUE : AL_FALSE);
	}
	void ASource_depAL::setPosition(const Vec3& pos) {
		ALEC_D(Trap, alSource3f, _id, AL_POSITION, pos.x, pos.y, pos.z);
	}
	void ASource_depAL::setDirection(const Vec3& dir) {
		ALEC_D(Trap, alSource3f, _id, AL_DIRECTION, dir.x, dir.y, dir.z);
	}
	void ASource_depAL::setVelocity(const Vec3& vel) {
		ALEC_D(Trap, alSource3f, _id, AL_VELOCITY, vel.x, vel.y, vel.z);
	}
	void ASource_depAL::setGainRange(float gmin, float gmax) {
		ALEC_D(Trap, alSourcef, _id, AL_MIN_GAIN, gmin);
		ALEC_D(Trap, alSourcef, _id, AL_MAX_GAIN, gmax);
	}
	void ASource_depAL::setAngleGain(float inner, float outer) {
		ALEC_D(Trap, alSourcef, _id, AL_CONE_INNER_ANGLE, inner);
		ALEC_D(Trap, alSourcef, _id, AL_CONE_OUTER_ANGLE, outer);
	}
	void ASource_depAL::setAngleOuterGain(float gain) {
		ALEC_D(Trap, alSourcef, _id, AL_CONE_OUTER_GAIN, gain);
	}
	...
	void SoundMgr_depAL::makeCurrent() {
		...
		ALCEC_D(Trap, alcMakeContextCurrent, _context[idx]);
	}
	void SoundMgr_depAL::resetCurrent() {
		UniLock lk(_mutex);
		ALCEC_D(Trap, alcMakeContextCurrent, nullptr);
	}
	void SoundMgr_depAL::suspend() {
		UniLock lk(_mutex);
		ALCEC_D(Trap, alcSuspendContext, _context[s_thID.get()]);
	}
	void SoundMgr_depAL::process() {
		UniLock lk(_mutex);
		ALCEC_D(Trap, alcProcessContext, _context[s_thID.get()]);
	}
}
