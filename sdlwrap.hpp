...
namespace rs {
	...
	class SpinLockRW {
		private:
			...
			using ThreadEnt_OP = spn::Optional<ThreadEnt>;
			...
			void _unlock() {
				...
				for(auto& e : _read) {
					AssertP(Trap, !e || e->threadId==*tls_threadID)
				}
				for(;;) {
			...
			void _unlockR() {
				...
				AssertP(Trap, !_write || _write->threadId==*tls_threadID)
				for(;;) {
					if(auto lk = _lockMutex()) {
						for(auto& e : _read) {
			...
			bool _findSetReadEnt() {
				spn::Optional<int> emptyIndex;
